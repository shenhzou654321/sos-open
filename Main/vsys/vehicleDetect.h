...
#ifndef VEHICLEDETECT_H
#define VEHICLEDETECT_H
...
extern "C" {
#endif
#define VehicleDetect_Mask 0x20LLU
    ...
    struct VehicleDetect {
        ...
        struct {
            ...
            pthread_t stid;
            /* 线程名 */
#define VehicleDetect_get_threadName(___pVehicleDetect) (((char*)(((struct VehicleDetect*)(___pVehicleDetect))+1))+((struct VehicleDetect*)(___pVehicleDetect))->ro._threadName_)
...
#define __VehicleDetectPrintf(___Printf,___format, ...)\
    ___Printf(1,___format",p=%p,runStatus=%hhd,tid=%ld\n  pMonitor=%p\n  externRunStatus=%p\n  parentTid=%ld\n  threadName:\"%s\"\n\
  pConfigManager=%p\n  itemList=%p/%lu\n",##__VA_ARGS__\
        ,pVehicleDetect,((struct VehicleDetect*)(pVehicleDetect))->rw.runStatus,((struct VehicleDetect*)(pVehicleDetect))->ro.stid\
        ,((struct VehicleDetect*)(pVehicleDetect))->ro._pMonitor,((struct VehicleDetect*)(pVehicleDetect))->ro._externRunStatus\
        ,((struct VehicleDetect*)(pVehicleDetect))->ro._parentTid,VehicleDetect_get_threadName(pVehicleDetect)\
        ,((struct VehicleDetect*)(pVehicleDetect))->ro._pConfigManager\
        ,((struct VehicleDetect*)(pVehicleDetect))->ro.__itemList_,((struct VehicleDetect*)(pVehicleDetect))->ro.__itemList_->nodeCount)
#define VehicleDetectPrintf(___format, ...) __VehicleDetectPrintf(FsPrintf,___format,##__VA_ARGS__)
#define VehicleDetectPrintf2(___format, ...) __VehicleDetectPrintf(FsPrintf2,___format,##__VA_ARGS__)

    /* 目标检测项 */
    struct VehicleDetect_item {
        ...
        struct {
            ...
            char *_uuid;
            /* 所属的车辆检测对象 */
            struct VehicleDetect *_pVehicleDetect;
            /* 整个通道依赖的所有数据的校验和 */
#define  VehicleDetect_item_ClusterLen  (FsPointer_to_long(&((struct VehicleDetect_item*) 0)->ro._sum)+sizeof(((struct VehicleDetect_item*) 0)->ro._sum))
        ...
        struct {
            ...
            /* 添加车牌识别任务的函数,使用__mutexCmdConnect互斥锁 */
            Recognition_item_add_plateRecognition_pthreadSafety__IO add_plateRecognition_pthreadSafety__IO;
            /* 添加车牌识别任务的函数,需要使用的对象,使用__mutexCmdConnect互斥锁 */
            struct Recognition_item *pRecognition_item;
            /* 记录卡口数据函数,使用__mutexCmdConnect互斥锁 */
            Record_item_add_watch_pthreadSafety__OI_2 add_watch_pthreadSafety__OI_2;
            /* 记录卡口数据函数,需要使用的对象,使用__mutexCmdConnect互斥锁 */
            struct Record_item * pRecord_item;
            /* 跟踪中的所有目标 */
            FsObjectList *__objectList_;
            ////////////////////////////////////////////////////////////////////
            /* 检测是否已初始化 */
            unsigned char hasInit : 1;
            /* 检测区域 */
            int x1, y1, x2, y2;
            /* 帧间隔 */
