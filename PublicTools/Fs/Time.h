...
#if !defined TIME_H && !defined JAVA_IN_C
...
extern "C" {
    ...
    /* 
     * 从一个字符串中获取时间,时间格式1:如"2014-05-01 12:00:00.000"或"2014-05-01/12:00:00.000"或"2014-05-01T12:00:00.000"的格式,后面的可以省略,如"2014"表示"2014-01-01 00:00:00.000",此时间是指0时区的时间;
     *     时间格式2:如"10.0",即为包含小数的数字,则直接转double返回;
    ...
    double fs_time_get_string2(const char string[], /* string的长度 */unsigned int stringLen);
    ...
    /* 把时间转为0时区的时间,返回占用rst_24的空间大小 */

    unsigned char fs_time_to_stringGMT(/* 储存结果的空间,2000年这样的不小于24个字节 */char rst_24[], /* 0时区的时间 */const double time);
    ...
    double fs_time_hw_get();

    /* 设置硬件时间,成功返回1,失败返回-1 */

    signed char fs_time_hw_set(double time);
