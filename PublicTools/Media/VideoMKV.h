 ...
 */
#if !defined VIDEOMKV_H && !defined JAVA_IN_C
...
#define VideoMKV_trackSubtitle 0x11
    ...
    struct VideoMKV_channel {
        ...
        union {
            ...
            struct {
                ...
                /* 视频高度 */
                unsigned int height;
                /* 视频内图像编码类型(1:mjpeg;2:h264;3:h265) */
                unsigned char encodeType;
                /* 帧率 */
                float frameRate;
    ...
    char* video_mkv_codecPrivate_h264_get__IO(unsigned int bufferLenth, /* h264数据 */const unsigned char frameBuffer[]);

    /* 获取h265流的编码信息,传入的数据必须是关键帧,返回编码信息数据(前4个字节为长度,第5个字节表示ContentEncodings中的长度) */

    char* video_mkv_codecPrivate_h265_get__IO(unsigned int bufferLenth, /* h265数据 */const unsigned char frameBuffer[]);
    ...
    int video_mkv_file_save_video_first(VideoMKV * const pMKV, /* 复制的文件指针 */FILE * const fd_copy);
    ...
    signed char video_mkv_h265_get_keyData(/* 结果的储存空间,前4字节为数据大小,后跟数据,空间比privateCode占用的空间小(包含头),可把privateCode传入,实现privateCode同keydata的转换 */unsigned char rst[],
            /* 为结构struct VideoMKV_track中的codecPrivate字段 */unsigned char codePrivate[]);
    ...
    long video_mkv_read_h265Stream_from_file(/* 读取到的数据的储存空间,关键帧大小不要小于此值+codecPrivate的长度(不含本文件在前面加的4个字节的长度),非关键帧长度就是此值 */unsigned char rst_buffer[],
            /* 关键帧的头数据信息,为空表示要读取的数据非关键帧,用video_mkv_h265_get_keyData获取 */ unsigned char keyData[],
            /* 数据储存的文件,不能为空 */FILE *fd,
            /* 数据储存位置相对于文件的绝对偏移量 */unsigned long long offset,
            /* 数据在文件中的长度 */unsigned int lenth);
