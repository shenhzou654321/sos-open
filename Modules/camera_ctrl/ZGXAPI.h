...
{
    CAM_FACTOR_HIKVISION,                                //海康威视相机(白光)
    CAM_FACTOR_HIKVISIONRED,                             //海康威视相机(红外)      
    CAM_FACTOR_ZGXLIGHT,                                 //中光学白光相机
    CAM_FACTOR_ZGXRED,                                   //中光学红光相机
    CAM_FACTOR_SHSLIGHT,                                 //上赛光电仪 (白光)
    CAM_FACTOR_SHSRED,                                   //上赛光电仪 (红外)   
    ...
    CAM_FACTOR_UNKNOWN,                                    //置晨海信流媒体服务器文件点播
    CAM_FACTOR_ONVIF                                        //支持onvif协议的相机(这类相机都是统一的控制方式)       
}camera_t;
...
typedef enum 
{
    ...
    CAM_PARAM_PRESET_OPT_SET,                                    //设置预置位
    CAM_PARAM_PRESET_OPT_GOTO                                     //转到预置位
} camera_param_t;
...
typedef struct 
{
    ...
    int                              nshotType;          //镜头类型(0:反比例 1：正比例)
    UINT                             nparam;             //nparam==0只设置pt， nparam==1只设置z        
    UINT                             nRes[17];           //预留字节
} CTS_PTZ, *PCTS_PTZ;
...
cts_res_t camera_ctl_setParam(int socket, camera_t camera, camera_param_t cmd, void* data, struct timeval* time_out);
cts_res_t camera_ctl_getParam(int socket, camera_t camera, camera_param_t cmd, void* data, struct timeval* time_out);
...
unsigned short InttoHexSmallThanA(WORD data);
#endif
